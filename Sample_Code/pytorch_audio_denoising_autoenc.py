...
class Convautoenc(nn.Module):
   def __init__(self):
      ...
      self.conv1=nn.Conv1d(in_channels=1, out_channels=64, kernel_size=2048, stride=512, padding=1023, bias=True) #Padding for 'same' filters (kernel_size/2-1)

      #Synthesis filter bank:
      self.synconv1=nn.ConvTranspose1d(in_channels=64, out_channels=1, kernel_size=2048, stride=512, padding=1023, bias=True)
...
if __name__ == '__main__':
    ...
    for run in range(1):
       ...
       if run==0:
          ...
          """
          x=x[:len(x)//2] #keep first half
          fs, xbuff= wav.read('test3.wav')
          x=np.append(x, xbuff[:len(xbuff)//2]) #append first half of next file
          """
       if run==1:
          #fs, x= wav.read('test3.wav')
          #"""
          x=x[len(x)//2:] #keep second half
          fs, xbuff= wav.read('test.wav')
          x=np.append(x, xbuff[len(xbuff)//2:]) #append second half of next file
          #"""
       ...
       #Xnoise=X+torch.randn(X.size())*0.1 #to avoid learning the specific noise, it needs to be varied in the optimization
       Ypred=model(X)
       #Ypred=Ypred.detach()
       outputlen=len(Ypred[0,0,:]) #length of the signal at the output of the network.
       print("outputlen=", outputlen)
       Y=X[:,:,:outputlen]  #the clean target signal with same length as model output
      ...
      #optimization of weights using method of random directions:
          optimrandomdir_pytorch.optimizer(model, loss_fn, X, Y, iterations=1000, startingscale=0.1, endscale=0.0)
          ...
          for epoch in range(2000):
             ...
             #"""
             Xlast=X[:,:,-1].clone() 
             X[:,:,1:]=X[:,:,:-1].clone() #round Robbin, shift 1 right
             X[:,:,0]=Xlast.clone()
             Ylast=Y[:,:,-1].clone() 
    ...
    Xnoise=(X+torch.randn(X.size())*0.05)
    ...
    os.system('espeak -ven -s 120 '+'"The noisy training input set for the autoencoder"')
    sound(2**14*xnoise,fs)
    ...
    x=x+(np.random.normal(scale=0.04, size=x.shape)) 
    
    os.system('espeak -ven -s 120 '+'"The verification set input to the autoencoder"')
    sound(2**14*x,fs)
